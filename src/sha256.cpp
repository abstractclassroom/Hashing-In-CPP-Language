...
std::array<std::uint8_t, 32> sha256_digest(const std::uint8_t* msg, std::size_t len) {
  std::uint32_t h[8] = {
  ...
  for (int i = 0; i < 8; ++i) {
    out[static_cast<std::size_t>(i) * 4 + 0] = static_cast<std::uint8_t>((h[i] >> 24) & 0xffu);
    ...
    out[static_cast<std::size_t>(i) * 4 + 3] = static_cast<std::uint8_t>((h[i] >> 0) & 0xffu);
  }
  return out;
}
...
SHA256::SHA256(std::string_view text)
    : digest_{sha256_digest(reinterpret_cast<const std::uint8_t*>(text.data()), text.size())} {}

SHA256::SHA256(std::istream& in) {
  auto bytes = read_all(in);
  digest_ = sha256_digest(bytes.data(), bytes.size());
}
